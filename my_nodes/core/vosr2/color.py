...
Modified by my-comfyui-nodes to support low-resolution low-frequency alignment.
"""
import math
...
def adain_color_fix(target: torch.Tensor, source: torch.Tensor) -> torch.Tensor:
    ...
    target_std = target.std(dim=[2, 3], keepdim=True, unbiased=False) + ADAIN_EPS
    source_mean = source.mean(dim=[2, 3], keepdim=True)
    source_std = source.std(dim=[2, 3], keepdim=True, unbiased=False) + ADAIN_EPS
...
def wavelet_color_fix(
    ...
    if downsample > 1:
        small = (
            max(1, target.shape[2] // downsample),
            max(1, target.shape[3] // downsample),
        )
        ...
        source_low = F.interpolate(
            source_low, size=output_size, mode="bilinear", align_corners=False
        )
        target_low = F.interpolate(
            target_low, size=output_size, mode="bilinear", align_corners=False
        )
    else:
