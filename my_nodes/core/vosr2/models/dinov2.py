# Modified by my-comfyui-nodes to add a bounded, device-safe position cache.
...
class _CachedPositionEmbedding(nn.Module):
    """A non-persistent buffer that follows its parent model across devices."""
...
class DinoVisionTransformer(nn.Module):
    def __init__(self, img_size=518, patch_size=14, embed_dim=1024, depth=24, num_heads=16, mlp_ratio=4.0):
        ...
        self._pos_embed_cache = nn.ModuleDict()
    ...
    def interpolate_pos_encoding(self, x, w, h):
        ...
        cache_key = f"{int(h)}_{int(w)}_{str(previous_dtype).replace('.', '_')}"
        cached = (
            self._pos_embed_cache[cache_key]
        ...
        result = torch.cat((class_pos_embed.unsqueeze(0), patch_pos_embed), dim=1).to(
            previous_dtype
        )
        if len(self._pos_embed_cache) >= 8:
            self._pos_embed_cache.clear()
        self._pos_embed_cache[cache_key] = _CachedPositionEmbedding(result.detach())
        return result
