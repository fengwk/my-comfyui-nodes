# Modified by my-comfyui-nodes to add SageAttention and device-safe RoPE caching.
...
def vosr_attention_backend() -> str:
    """Return the eager DiT attention backend available in this ComfyUI build."""
    if SAGE_ATTENTION_IS_AVAILABLE and attention_sage is not None:
        return "sage"
    return "sdpa"


def _vosr_attention(q, k, v, heads):
    """Use ComfyUI's SageAttention adapter, with its own safe SDPA fallback."""
    ...
    use_sage = (
        SAGE_ATTENTION_IS_AVAILABLE
        and attention_sage is not None
        and not compiling
    )
    if not _ATTENTION_BACKEND_LOGGED:
        backend = "sage" if use_sage else "sdpa"
        logging.info("[TE-Speed-VOSR2] DiT attention backend: %s", backend)
...
class _DynamicRope(nn.Module):
    """Device-movable, non-persistent RoPE tensors for one latent geometry."""
    ...
    def forward(self, value: torch.Tensor) -> torch.Tensor:
        ...
        return value * self.freqs_cos + rotate_half(value) * self.freqs_sin
...
class LightningDiT(nn.Module):
    ...
    def __init__(
        ...
        # ModuleDict makes cached tensors follow ModelPatcher device moves. A
        # plain closure cache would retain CUDA tensors after staged offload.
        self._dynamic_rope_cache = nn.ModuleDict()
    ...
    def _get_dynamic_rope(self, hw_seq_len, device, dtype):
        ...
        cache_key = f"{int(hw_seq_len)}_{str(dtype).replace('.', '_')}"
        cached = (
            self._dynamic_rope_cache[cache_key]
            if cache_key in self._dynamic_rope_cache
            else None
        )
        ...
        cached = _DynamicRope(freqs_cos, freqs_sin)
        if len(self._dynamic_rope_cache) >= 4:
            self._dynamic_rope_cache.clear()
        self._dynamic_rope_cache[cache_key] = cached
        return cached
